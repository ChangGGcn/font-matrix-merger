...
- **跨设计空间合成**：打底可变时，其逐字形增量会被**重参数化到合并后的归一化空间**（`format/axis_mapping.py`），使合并字形在任意位置都与打底在该位置的实例逐点一致——支持打底独有轴、不同轴范围与不同 `avar`。打底默认点与合并默认点不重合时，缺的 master 由幽灵点插值补出并重定基；网格自检（`verify_composition`）不通过则回退到旧的"按打底默认实例化"行为。
